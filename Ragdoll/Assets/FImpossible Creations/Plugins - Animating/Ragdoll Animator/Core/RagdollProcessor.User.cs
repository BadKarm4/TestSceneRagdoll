...
namespace FIMSpace.FProceduralAnimation
{
    public partial class RagdollProcessor
    {
        ...
        /// <param name="duration"> Time in seconds </param>
        public IEnumerator User_SetPhysicalTorque(Vector3 rotationPower, float duration)
        {
            float elapsed = -0.0001f;
            WaitForFixedUpdate fixedWait = new WaitForFixedUpdate();
            ...
            while (elapsed < duration)
            {
                ...
                while (c != null)
                {
                    if (c.rigidbody) c.rigidbody.AddTorque(rotationPower, ForceMode.Impulse);
                    c = c.child;
                }
        ...
        public bool RecognizeRagdollIsStandingOnLegs(float tolerance = 0.75f, Vector3? worldUp = null)
        {
            ...
            Vector3 footsMidToHips = posingLeftLowerLeg.transform.position;
        ...
        {
            Vector3 footMiddle = posingLeftLowerLeg.transform.position;
        ...
        /// </summary>
        public EGetUpType User_CanGetUp(Vector3? worldUp = null, bool canBeNone = true, bool includeLeftRightSide = false, float tolerance = 0.35f)
        {
        ...
        {
            float refScale = Vector3.Distance(posingPelvis.transform.position, posingHead.transform.position) * 0.2f;
